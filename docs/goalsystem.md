...
2. `weekly`: target repeats each week in the date range.
...
4. Step 4 is partially implemented: heading-focus handoff, keyboard focus styles, and 44px tap-target minimums are in place; full manual accessibility/UX QA remains in progress.
...
7. Prevent implicit form submission from creating goals before Review; only explicit `Save goal` click should trigger create.
