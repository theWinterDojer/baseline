...
## Post-QA Escrow Hardening Queue
- Add a tracked pledge-level contract address reference in DB/app (`escrow_contract_address`) so mixed-version reconciliation/operations are explicit.
- Add server-side completion threshold enforcement to mirror UI completion gating (prevent non-UI bypass of early completion writes).
- Add on-chain/off-chain reconciliation job to compare pledge status and settlement metadata for drift detection.
...
- Pledge
  - id, goalId, sponsorId, amount, deadline, minCheckIns, status, acceptedAt, approvalTimestamp, settledAt, escrowTx, onchainPledgeId, escrowChainId, escrowTokenAddress, escrowAmountRaw, settlementTx
- SponsorCriteria
  - pledgeId, text
- Comment
